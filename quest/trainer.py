...
class QueSTTrainer:
    def __init__(self, dataset, data_path, sample_ids, adata_list, query_sample_id=None, query_niches=None,
        ...
        logger.info(f"QueST Trainer initialized, loading data")
        ...
        logger.info(f"Data loaded with {len(self.adata_list)} adata objects")
    ...
    def train_model(self):
        ...
        self.model.train()
        epoch_pbar = tqdm(range(self.model_param['epochs']), desc='training', unit='epoch')
        for epoch in epoch_pbar:
            ...
            for batch_idx, i in enumerate(indices):
                ...
                logger.info(f"Epoch: {epoch}, Batch: {batch_idx + 1}/{len(self.adata_list)}, Sample: {adata.uns['library_id']}")
    ...
    def query(self, q_id, q_niche_mask, q_niche_name):
        logger.info(f"Performing query: q_id={q_id}, q_niche={q_niche_name}")
    ...
    def get_embedding(self):
        logger.info(f"Saving niche embeddings for each sample")
        with torch.no_grad():
    ...
    def _setup(self):
        """Preprocess + build features + init model (idempotent)."""
    ...
    def inference(self, ckpt_path=None, save_embedding=False, query=False):
        self._setup()
